...
#### Show Command
- **Behavior**: Passes through without capturing, but clears `TF_CLI_ARGS` to prevent globally set CLI arguments from interfering
...
│   ├── plan_output_raw.log   # Generated by wrapper
│   └── terragrunt.hcl
├── module2/
│   ├── main.tf
...
- Uses POSIX `/bin/sh` for compatibility with minimal containers (e.g., Alpine)
- Sets `set -e` for proper error propagation
- Uses `tee` to capture logs while displaying output
- Preserves exit codes using a temporary file (`/tmp/wrapper_exitcode_$$`) for POSIX compliance
...
- **Solution**: Check that the temp file mechanism (`/tmp/wrapper_exitcode_$$`) is preserving exit codes correctly
